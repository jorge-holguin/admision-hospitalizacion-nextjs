...
import useEmergencia, { EmergenciaRecord } from "./hooks/useEmergencia" // Import the custom hook
...
export default function AdmissionPage() {
  ...
  // Usar el hook personalizado para obtener los datos de emergencia
  const { emergencias, loading, error, fetchEmergencias } = useEmergencia()
  
  // Estado para almacenar los registros de emergencia formateados para la UI
  const [emergenciaRecords, setEmergenciaRecords] = useState<any[]>([
    {
      estado: "3",
      emergencia: "25033385",
      fecha: "30/07/2025",
  ...
  // Efecto para transformar los datos de la API al formato que espera la UI
  useEffect(() => {
    if (emergencias && emergencias.length > 0) {
      // Transformar los datos de la API al formato que espera la UI
      const formattedRecords = emergencias.map(record => ({
        estado: record.Estado || "3",
        emergencia: record.Emergencia_id,
        ...
        sexo: record.Sexo,
        seguro: record.Nombre_Seguro || "",
        consultorio: record.Consultorio || "",
        nombreConsultorio: record.Nombre_Consultorio || "",
        motivo: record.Nombre_motivo || "",
        usuario: record.Usuario,
        tipoAtencion: record.TipoAtencion || "E",
        // Campos adicionales para el formulario (se mantendrán vacíos hasta que se edite)
        fNacimiento: "",
        apellidoPaterno: "",
        apellidoMaterno: "",
        id: record.Emergencia_id,
        edad: "",
        estadoCivil: "",
      ...
      setEmergenciaRecords(formattedRecords);
    }
  }, [emergencias])
